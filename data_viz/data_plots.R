...
origin_data <- origin_data %>% mutate(target_categ = ifelse(target == "Graduate",
                                                           "Success", "Fail"))

#### Unidades curriculares aprovadas en primer y segundo semestre ####

units_approved_1_plot <- ggplot(origin_data, aes(x = origin_data$target_categ,
                                                 y = origin_data$`curricular_units 1st sem (approved)`)) + 
  geom_violin(aes(color=target_categ), trim = TRUE) +
  labs(title = "Curricular units approved in 1st semester", y = "Number of units", x = "") + 
  theme_classic() +
  geom_signif(comparisons = list(c("Success", "Fail")),   
              map_signif_level=TRUE)

units_approved_2_plot <- ggplot(origin_data, aes(x = origin_data$target_categ,
...
units_grade_1_plot <- ggplot(origin_data, aes(x = origin_data$target_categ,
...
units_grade_2_plot <- ggplot(origin_data, aes(x = origin_data$target_categ,
...
course_plot <- ggplot(origin_data,aes(x = course)) + 
  ...
  geom_histogram(data=subset(origin_data, target_categ == 'Success'),fill = "green", alpha = 0.25) +
  geom_histogram(data=subset(origin_data, target_categ == 'Fail'),fill = "red", alpha = 0.25)

course_counts <- origin_data %>% group_by(target_categ, course) %>% count()
...
application_counts <- origin_data %>% group_by(target_categ, application_mode) %>% count()

#### Edad al momento de ingreso ####

age_plot <- ggplot(origin_data, aes(x = target_categ, y = `age_at enrollment`)) + 
  geom_violin(aes(color=target_categ), trim = TRUE) +
...
units_woeval_1_plot <- ggplot(origin_data, aes(x = target_categ, y = `curricular_units 1st sem (without evaluations)`)) + 
  geom_violin(aes(color=target_categ), trim = TRUE) +
...
units_woeval_2_plot <- ggplot(origin_data, aes(x = target_categ, y = `curricular_units 2nd sem (without evaluations)`)) + 
  geom_violin(aes(color=target_categ), trim = TRUE) +
...
inflation_plot <- ggplot(origin_data, aes(x = target_categ, y = inflation_rate)) + 
  geom_violin(aes(color=target_categ), trim = TRUE) +
